...
// Internal error codes written to the CSV "Status Code" column when a link fails
// before an HTTP response is received. They live in the 1001-1099 range so they
// can never collide with real HTTP status codes.
const (
	errCodeRequestConstruction = 1001 // building the HTTP request failed
	errCodeRequestFailed       = 1002 // the HTTP request could not be completed
	errCodeProcessingPanic     = 1003 // processing the link panicked
)
